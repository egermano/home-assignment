...
var main = {
	...
	init: function(){
		...
		this.sortList();
		this.splitList();
		this._populateContent(this.data[0].name, this.data[0].content);
	},
	sortList: function(){
		this.data.sort(function (a, b) {
			if (a.name.slugfy() > b.name.slugfy()) return 1;
			if (a.name.slugfy() < b.name.slugfy()) return -1;
			return 0;
		});
	},
	splitList: function(){
		...
		for(var i = 0; i<3; i++) {
			...
			if (total === 4) {
				if (rest === 1 && i === 1) ends--;
				else if (rest === 1 && i === 2) starts--;
			}

			var items = tempData.splice(starts, ends),
	...
	_createList: function(items){
		...
		for(var i = 0; i<items.length; i++) {
			...
			link.onclick = function(){
				main._populateContent(this.name, this.content);
				return false;
			}.bind(items[i]);
	...
	_populateContent: function(name, content) {
		this.cityContent.innerHTML = "<h3>"+name+"</h3>" + content;
	}
};
